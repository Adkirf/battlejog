...
export function createState(inParentComponent: React.Component) {
  ...
  return {
    ...
    initUser : function(username: string){

      this.setState({socketComm: <WebSocket>createSocketComm(inParentComponent, username) })
    ...
    handleMessage_session: function(inLayout: number[][][], score: number, opponentscore: number){
      const scores: IScores = {player:score, opponent: opponentscore}
      this.setState({
        layout: inLayout,
        scores: scores,
        gameState: "playing"
    ...
    tileClick : function(inLayer: number, inRow: number, inColumn: number) {
      ...
      if (selectedTiles.length === 2) {
        if (selectedTiles[0].type === selectedTiles[1].type ||
          ...
          const p: any = scores.player;
          const i: any = calculatedPoints;
          const n: number = parseInt(p) + parseInt(i);
          scores.player = n;
          timeSinceLastMatch = now;
          console.log(`WS sending: ${`match_${this.state.pid}_${calculatedPoints}`}`);
          this.state.socketComm.send(`match_${this.state.pid}_${calculatedPoints}_${JSON.stringify(layout)}`);
    ...
    quitGame : function(){
      this.state.socketComm.send(`quit_${this.state.pid}`);
    }.bind(inParentComponent)
